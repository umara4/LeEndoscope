"""
VideoWindow -- thin coordinator for the surgical imaging interface.

Owns backend services and child widgets, wires signals between them.
Split from video_window.py (the 3070-line monolith).
...
class VideoWindow(QMainWindow, DebouncedGeometryMixin):
    def __init__(self, patient_id: str = None, patient_db=None):
        ...
        if patient_id and patient_db:
            try:
                ...
                display = p.get_display_name() if p else f"Patient {patient_id[:8]}"
                self.setWindowTitle(f"Surgical Imaging Interface - {display}")
            except Exception:
        ...
        # Side panel
        self._side = SidePanel(has_patient=bool(patient_id))
    ...
    # ------------------------------------------------------------------
    # Geometry persistence (using DebouncedGeometryMixin)
    # ------------------------------------------------------------------
